...
# 导入自定义的drawRectBox函数
try:
    from my_image_utils import drawRectBox, imRectBox  # 导入自定义的函数
except ImportError:
    from QtFusion.utils import drawRectBox  # 如果导入失败，继续使用原来的函数
from QtFusion.utils import get_cls_color  # 从QtFusion库导入get_cls_color函数，用于获取类别颜色
from PySide6 import QtWidgets, QtCore  # 导入PySide6库的QtWidgets和QtCore模块，用于创建GUI
from QtFusion.path import abs_path
from QtFusion.config import QF_Config
from YOLOv8v5PlateModel import YOLOv8v5PlateDetector  # 从YOLOv8Model模块导入YOLOv8Detector类，用于物体检测
