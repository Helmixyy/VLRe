...
from QtFusion.utils import cv_imread, drawRectEdge
from QtFusion.widgets import QMainWindow, QWindowCtrls, updateTable, replaceWidget, QImageLabel
from QtFusion.utils.ImageUtils import *  # 保留原来的导入
try:
    from my_image_utils import drawRectBox, imRectBox, drawRectEdge, imRectEdge  # 导入自定义的函数
except ImportError:
    pass  # 如果导入失败，将继续使用原来的函数

from Recognition_UI import Ui_MainWindow
from YOLOv8v5PlateModel import YOLOv8v5PlateDetector, count_classes
...
class RecMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None, *args, **kwargs):
        ...
        QWindowCtrls(self, exit_title="基于YOLOv8v5/LPRNet的车牌检测系统",  # 退出确认框标题
        ...
        # 加载模型
        self.model = YOLOv8v5PlateDetector()  # 创建YOLOv8Detector模型实例
        # 加载训练的模型权重
        yolo_model_path = abs_path('weights/best-yolov8n.pt')
        lpr_model_path = abs_path('weights/Final_LPRNet_model.pth')
        self.model.load_model(yolo_model_path, lpr_model_path)
    ...
    def slot_init(self):
        ...
        self.toolButton_author.clicked.connect(self.toggle_about)  # 点击作者按钮时触发toggle_about槽函数
            ...
            if col == 0:  # 只在点击第一列时执行操作
                ...
                if (this_path is not None) & (res is not None) & (axes is not None):
                    ...
                    if os.path.isfile(this_path):  # 检查文件路径是否存在
                        ...
                        confi = self.tableWidget.item(row, 4).text()  # 获取置信度值
    ...
    def toggle_comboBox(self):
        ...
        if len(self.detInfo) > 0:  # 检查检测结果列表是否为空
            ...
            for i, det in enumerate(self.detInfo):  # 遍历所有标记框
                ...
                plate_img = self.current_image[ymin:ymax, xmin:xmax]

                # 在界面中显示车牌图像
                self.dispImage(self.label_bar, plate_img)
                self.label_plate_str.setText(det['plate_number'])  # 显示车牌识别的字符
                self.label_plate_color.setText(det["plate_color"])  # 显示车牌颜色

                # 在图像上标记目标框
                name, bbox, conf, cls_id, plate_number, plate_color = (det['class_name'], det['bbox'],
                                                                       det['score'], det['class_id'],
                                                                       det['plate_number'], det['plate_color'])
    ...
    def frame_process(self, image):
        ...
        if det is not None and len(det):
            ...
            if len(det_info):
                ...
                name, bbox, conf, cls_id, plate_number, plate_color = det_info[0]['class_name'], det_info[0]['bbox'], \
                    det_info[0]['score'], det_info[0]['class_id'], det_info[0]['plate_number'], det_info[0][
                    'plate_color']
                self.label_class_result.setText(name)  # 显示类别
                self.label_plate_str.setText(plate_number)  # 显示车牌识别的字符
                self.label_plate_color.setText(plate_color)  # 显示车牌颜色
                ...
                for info in det_info:  # 遍历检测信息
                    name, bbox, conf, cls_id, plate_number, plate_color = (info['class_name'], info['bbox'],
                                                                           info['score'], info['class_id'],
                                                                           info['plate_number'], info['plate_color'])
                    if plate_number:
                        # 从原图中剪辑车牌图像
                        xmin_p, ymin_p, xmax_p, ymax_p = bbox
                        plate_img = self.current_image[ymin_p:ymax_p, xmin_p:xmax_p]
                        self.plate_imgs.append(plate_img)  # 将车牌图像添加到列表中
                    ...
                    # 更新表格
                    rec_conf = plate_number if plate_number else '%.2f' % conf
                    updateTable(self.tableWidget, self.id_tab, self.file_path, name, bbox, rec_conf)
                    self.id_tab += 1
    ...
    def toggle_version(self):
        ...
        版本信息
        """
        QMessageBox.about(self.centralwidget, "版本信息", "车牌识别系统")

    def toggle_about(self):
        """
        关于系统
        """
        QMessageBox.about(self.centralwidget, "关于系统", "基于YOLOv8v5/LPRNet的车牌检测系统")
